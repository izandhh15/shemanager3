...
class SeedReferenceData extends Command
{
    protected $signature = 'app:seed-reference-data
                            ...
                            {--country= : Seed only a specific country (e.g., ES, ESF)}
                            {--season=2025 : Season data folder to use (e.g., 2025, 2026)}';

    protected $description = 'Seed teams, competitions, fixtures, and players from season JSON data files';

    /** Active season folder (e.g., "2025" or "2026") */
    private string $activeSeason = '2025';
    ...
    public function handle(): int
    {
        ...
        $this->activeSeason = $this->option('season') ?? '2025';
        ...
        $this->info("Using profile: {$profile}, season: {$this->activeSeason}");
        ...
        // Determine which countries to seed based on profile and season
        $isWomensSeason = $this->activeSeason === '2026';
        $countryCodes = match (true) {
            $profile === 'test'  => ['XX'],
            $isWomensSeason      => $countryConfig->womenPlayableCountryCodes(),
        ...
        if ($countryFilter) {
            ...
            $allKnown = array_merge(
                $countryConfig->playableCountryCodes(),
                $countryConfig->womenPlayableCountryCodes(),
                ['XX']
            );
            if (!in_array($countryFilter, $allKnown)) {
                $this->error("Country '{$countryFilter}' not found in any profile.");
                return CommandAlias::FAILURE;
            }
    ...
    private function seedCountry(string $countryCode, CountryConfig $countryConfig): void
    {
        ...
        foreach ($tiers as $tier => $tierConfig) {
            $this->seedCompetition([
                ...
                'path' => "data/{$this->activeSeason}/{$tierConfig['competition']}",
                'tier' => $tier,
        ...
        foreach ($cupIds as $cupId) {
            ...
            $this->seedCompetition([
                'code' => $cupId,
                'path' => "data/{$this->activeSeason}/{$cupId}",
                'tier' => 0,
        ...
        foreach ($transferPool as $code => $poolConfig) {
            ...
            $this->seedCompetition([
                'code' => $code,
                'path' => "data/{$this->activeSeason}/{$code}",
                'tier' => 1,
    ...
    private function seedCupCompetition(string $basePath, string $code, int $tier, string $handler, string $country, string $flag, string $role = 'domestic_cup'): void
    {
        ...
        // Seed cup teams (link existing teams to cup) — support enters_round field
    ...
    private function seedSwissFormatCompetition(string $basePath, string $code, int $tier, string $handler, string $country, string $flag, string $role = 'european'): void
    {
        ...
        $season = $teamsData['seasonID'] ?? $this->activeSeason;
    ...
    private function seedCupTeams(array $clubs, string $competitionId, string $season, string $country = 'ES'): void
    {
        ...
        foreach ($clubs as $club) {
            ...
            // Support both the old supercup-based entry round logic and the new
            // explicit enters_round field used in women's competition JSON files.
            // Priority: explicit enters_round in JSON > supercup calculation > default 1
            if (isset($club['enters_round'])) {
                // Map named rounds to numeric rounds
                $entryRound = $this->resolveEntryRound($club['enters_round']);
            } else {
    ...
    /**
     * Resolve a named round string to its numeric value.
     * Supports both numeric strings and named round identifiers.
     */
    private function resolveEntryRound(string|int $round): int
    {
        if (is_int($round)) {
            return $round;
        }
        ...
        return match ($round) {
            ...
            'cup.round_of_32' => 2,
            'cup.round_of_16' => 3,
            'cup.quarter_finals' => 4,
            'cup.semi_finals' => 5,
            'cup.final' => 6,
            default => is_numeric($round) ? (int) $round : 1,
        };
    }
