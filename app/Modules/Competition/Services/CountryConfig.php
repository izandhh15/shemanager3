...
class CountryConfig
{
    ...
    /**
     * Get playable women's country codes only.
     *
     * @return string[]
     */
    public function womenPlayableCountryCodes(): array
    {
        $womenCodes = ['ESF', 'DEF', 'ENF', 'FRF', 'ITF'];
        return collect($this->allCountries())
            ->filter(fn (array $config, string $code) => in_array($code, $womenCodes) && !empty($config['tiers']) && empty($config['tournament']))
            ->keys()
            ->all();
    }
    ...
    public function flag(string $countryCode): string
    {
        return match ($countryCode) {
            'EN'  => 'gb-eng',
            // Women's federation codes map to their country flag
            'ESF' => 'es',
            'DEF' => 'de',
            'ENF' => 'gb-eng',
            'FRF' => 'fr',
