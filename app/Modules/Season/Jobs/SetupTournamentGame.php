...
class SetupTournamentGame implements ShouldQueue
{
    ...
    public function handle(
        ...
        $groupsPath = base_path('data/2026/WWC2027/groups.json');
        ...
        // WWC2027 team_mapping has a confederation-nested structure:
        // { UEFA: { nations_ranked: [{code, name, ...}] }, CONMEBOL: {...}, ... }
        // Flatten it to a code → team record map compatible with the rest of the job.
        $flatMapping = [];
        $reservedKeys = ['_meta', 'final_32_qualified', 'interconfederation_playoffs'];
        foreach ($mapping as $key => $confData) {
            if (in_array($key, $reservedKeys)) {
                continue;
            }
            if (is_array($confData) && isset($confData['nations_ranked'])) {
                foreach ($confData['nations_ranked'] as $nation) {
                    $code = $nation['code'] ?? null;
                    if (!$code) continue;
                    // Look up team by country code (type=national)
                    $team = \DB::table('teams')
                        ->where('country', $code)
                        ->where('type', 'national')
                        ->first();
                    if ($team) {
                        $flatMapping[$code] = [
                            'uuid'            => $team->id,
                            'name'            => $nation['name'] ?? $team->name,
                            'transfermarkt_id'=> null,
                            'is_placeholder'  => $nation['is_placeholder'] ?? false,
                        ];
                    }
                }
            }
        }
        $teamKeyMap = collect($flatMapping)->mapWithKeys(
        ...
        $currentDate = $game->current_date ?? Carbon::parse('2027-07-02');
        $this->createGamePlayers($flatMapping, $developmentService, $currentDate);
    ...
    private function createGamePlayers(array $teamMapping, PlayerDevelopmentService $developmentService, Carbon $currentDate): void
    {
        ...
        $basePath = base_path('data/2026/WWC2027/teams');
        $allPlayers = Player::all()->keyBy('transfermarkt_id');
        $playerRows = [];

        // WWC2027 roster files are named by FIFA code (ESP.json, ENG.json, etc.)
        // Process all teams that have a roster file
        foreach ($teamMapping as $fifaCode => $teamData) {
            $filePath = "{$basePath}/{$fifaCode}.json";
